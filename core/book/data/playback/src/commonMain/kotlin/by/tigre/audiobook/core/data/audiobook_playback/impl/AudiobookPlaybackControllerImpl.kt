...
internal class AudiobookPlaybackControllerImpl(
    ...
    private var shouldRewindOnResume: Boolean = true

    /**
     * Listened time (ms) in the book at a reference position (e.g. after chapter change); blocks persisting an earlier
     * position until [mayPersistBelowCanonical] or until caught up.
     */
    ...
    private suspend fun setChapter(chapter: Chapter, positionMs: Long) {
        ...
        player.setMediaItem(
            MediaItemWrapper(
                uri = chapter.fileUri,
                title = chapter.title,
                albumTitle = currentBook.value?.title
            ),
            positionMs
        )
    }
    ...
    private suspend fun applyRewindBeforePlaybackResume() {
